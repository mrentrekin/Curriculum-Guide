(() => {
  ...
  const addPage = (id, title, kicker, html, searchExtra = "") => {
    pages.push({
      id,
      title,
      kicker,
  ...
  const sectionHTML = (section, prefix = "") => {
    ...
    switch (section.key) {
      ...
      case "sequence":
        return `${title}${tableHTML({
          headers: ["Days", "Topic", "Skills & concepts", "Activities"],
          rows: (section.rows || []).map((r) => [r.days, r.topic, r.skills, r.activities]),
        })}`;
      case "vocabulary":
        return `${title}${tableHTML({
  ...
  addPage(
    ...
    "Architecture",
    `
      <p class="kicker">${escape(course.lesson.length)}</p>
      <h1>The daily lesson</h1>
      <p class="lede">${escape(course.lesson.note)}</p>
      ${tableHTML({
  ...
  pages.forEach((p, i) => {
    ...
    nav.innerHTML = `
      ${prev && prev.id !== "search" ? `<a href="#${prev.id}"><span>Previous</span>${escape(prev.title)}</a>` : "<span></span>"}
      ${next && next.id !== "search" ? `<a href="#${next.id}"><span>Next</span>${escape(next.title)}</a>` : ""}
    `;
  });
